    ...
    items.forEach((item) => {
      ...
      if (!hasOwnProp.call(item, "longname")) {
        ...
        itemsNav += [
          ...
          hasMethods ? [`<ul class="has-class-methods">`, methods.map((meth) => `<li>${getIcon(meth.kind)}${linktoFn(meth.longname, meth.name)}</li>`).join(""), members.map((meth) => `<li>${getIcon(meth.kind)}${linktoFn(meth.longname, meth.name)}</li>`).join(""), `</ul>`].join("") : "",
          `</li>`,
        ].join("");
